...
import { initialAgents, initialListings, initialReports } from "../data";
...
export default function ReportsView() {
  ...
  const [notesList, setNotesList] = useState<Record<string, string[]>>({
    RPT001: [
      "Admin Officer: Pattern of requesting upfront payment. Highly suspicious pattern.",
      "System: Secondary audit triggered due to past reports threshold (5)",
    ],
  });
  const [activeModal, setActiveModal] = useState<
    "ban" | "reduce" | "suspend" | null
  >(null);
  const [modalTargetId, setModalTargetId] = useState<string | null>(null);
  const [modalReason, setModalReason] = useState("");
  const [scoreReduction, setScoreReduction] = useState(10);

  const openModal = (type: "ban" | "reduce" | "suspend", agentId: string) => {
    setActiveModal(type);
    ...
    setAgents((current) =>
      current.map((agent) => {
        ...
        if (activeModal === "suspend")
          return { ...agent, verificationStatus: "Blocked" };
        return {
          ...agent,
  ...
  if (selectedReport) {
    ...
    const relatedListing = listings.find(
      (l) => l.id === selectedReport.relatedListingId,
    );
        ...
        <div className="bg-white p-5 rounded-xl border border-emerald-950/5 flex flex-col gap-4 shadow-xs">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`px-2 py-0.5 text-[9px] font-medium rounded-md tracking-wide ${
                  selectedReport.severity === "High"
                    ? "bg-red-100 text-red-500 shadow-sm shadow-red-500/15"
              ...
              <span
                className={`px-2 py-1  text-[10px] font-medium rounded-lg ${
                  selectedReport.status === "Resolved"
                    ? "bg-emerald-500 text-emerald-800 border border-emerald-100"
          ...
          <div className="grid grid-cols-4 gap-4 border-t border-b border-slate-50 py-3.5 text-xs bg-slate-100 px-3 rounded-lg">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-slate-400 font-medium block mb-1  text-[10px]">
                  Reporter
                </span>
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div className="bg-white p-5 rounded-xl border border-emerald-950/5 space-y-4">
              ...
              <div className="space-y-1.5 pt-1.5">
                <span className=" flex items-center gap-2 text-[10px] text-slate-400 font-semibold  block">
                  <MessageSquare size={14} /> Complaint Description
                </span>
                    ...
                    <div className="">
                      <div className="mb-2">
                        <h5 className="text-xs font-bold text-slate-800 leading-tight">
                          {selectedReport.relatedListingName ||
                            "Listed Property Flat"}
                        </h5>
                        <p className="text-[10px]  font-bold text-slate-400 mt-1">
                          ID: {selectedReport.relatedListingId} •{" "}
                          {selectedReport.relatedListingPrice || "₦1.2M/yr"}
                        </p>
                      </div>
                      <button
                        onClick={() => {
            ...
            <div className="bg-white p-5 rounded-xl border border-slate-200/60 overflow-hidden text-xs space-y-4">
              ...
              <div className="space-y-3">
                {selectedReport.previousOffenses &&
                selectedReport.previousOffenses.length > 0 ? (
                  selectedReport.previousOffenses.map((offense) => (
                    ...
                    >
                      ...
                      <p className="text-green-600 bg-green-50 border border-green-100 p-1 rounded-se-lg font-bold   ">
                        Resolution:{" "}
                  ...
                  <p className="text-slate-400 py-3 text-center">
                    No record of previous offenses found against this profile.
                  </p>
                )}
              </div>
            </div>
            ...
            {relatedAgent && (
              <div className="bg-white p-5 rounded-xl border border-[#e2fef4] bg-red-50/10 space-y-4">
                ...
                <div className="pt-2 border-t border-slate-100 flex flex-col gap-2">
                  <button
                    disabled
                    className="text-[10px] font-medium text-slate-400 mt-1  bg-gray-100 py-2 px-2 rounded-lg text-left flex flex-col"
                  >
                    Trust score{" "}
                    <span className="text-red-600 font-bold">
                      {relatedAgent.trustScore}
                    </span>
                  </button>
                  ...
                  <div className=" flex gap-2 w-full">
                    <button
                      disabled
                      className="text-[10px] flex-1 font-medium text-slate-400 mt-1  bg-gray-100 py-2 px-2 rounded-lg text-left flex flex-col"
                    >
                      Reports{" "}
                      <p className="text-red-500 font-bold">
                        {selectedReport.pastReportsCount}
                      </p>
                    </button>
                    <button
                      disabled
                    ...
                    >
                      ...
                      <span className="text-slate-600 font-bold">
                        {relatedAgent.activeListingsCount}
                      </span>
                    </button>
                  </div>
                  <button
                ...
                <div className="bg-white p-5 rounded-xl border border-[#e2fef3]  space-y-4">
                  ...
                  <div className=" flex flex-col gap-2">
                    <div className="flex flex-col gap-2">
                      ...
                      <button
                        onClick={() => openModal("suspend", relatedAgent.id)}
                      ...
                      <button
                        onClick={() => openModal("reduce", relatedAgent.id)}
                    ...
                    <button
                      onClick={() => openModal("ban", relatedAgent.id)}
              ...
              <div className="space-y-2.5 max-h-48 overflow-y-auto">
                {reportNotes.length === 0 ? (
                  <p className="text-[11px] text-slate-400 p-2 text-center">
                    No internal notes logged. Write below.
                  </p>
                ) : (
                  reportNotes.map((note, i) => (
                    <div
                      key={i}
                      className="bg-slate-50 p-2 py-2.5 border border-slate-100 rounded-lg text-[11px] text-slate-600 leading-relaxed font-semibold  overflow-hidden"
                    >
            ...
            <tbody className="divide-y divide-slate-50 text-xs">
              {filteredReports.length === 0 ? (
                ...
                filteredReports.map((report) => (
                  <tr
                    key={report.id}
                    className="hover:bg-[#f4fcf8]/50 transition-all font-medium"
                  >
                    ...
                    <td className="py-3 px-5 text-slate-700 text-[10px]">
                      <span
                        className="font-bold text-[10px] hover:text-[#004d2c] cursor-pointer"
                    ...
                    <td className="py-3 px-5">
                      <span
                        className={`px-2 py-0.5 rounded-full text-[9px] font-extrabold  tracking-wide border ${

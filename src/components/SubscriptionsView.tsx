...
import { initialSubscriptions, subscriptionCard } from "../data";

export default function SubscriptionsView() {
  const [subscriptions, setSubscriptions] =
    useState<OgaSubscription[]>(initialSubscriptions);
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const [refreshCount, setRefreshCount] = useState(0);

  const filteredSubs = subscriptions.filter((sub) => {
    const matchesSearch = sub.agentName
      .toLowerCase()
    ...
    <div id="subscriptions-view" className="space-y-6">
      <div className="w-full ">
        ...
        <p className="text-[10px] text-slate-400">
          Manage agent subscription and payments
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {subscriptionCard.map((card) => (
          <div
            key={card.id}
            className=" p-2 border border-gray-200   bg-white rounded-md shadow"
          >
            <div className="">
              <div
            ...
            <h1 className="pt-2 text-sm font-bold text-green-800">
              {card.count}
            </h1>
            ...
            <tbody className="divide-y divide-slate-50 text-xs font-semibold">
              {filteredSubs.length === 0 ? (
                ...
                filteredSubs.map((sub) => (
                  <tr
                    key={sub.id}
                    className="hover:bg-[#f4fcf8]/50 transition-all"
                  >
                    ...
                    <td className="py-3 px-5">
                      <span
                        className={`px-2.5 py-0.5 rounded-full text-[9px] font-normal  tracking-wide border ${
                          sub.status === "Active"
                            ...
                            current.map((subscription) =>
                              subscription.id === sub.id
                                ? {
                                    ...subscription,
                                    status: "Active",
                                    endDate: `Dec 31, ${new Date().getFullYear() + 1}`,
                                  }
                                : subscription,
                            ),
                          )

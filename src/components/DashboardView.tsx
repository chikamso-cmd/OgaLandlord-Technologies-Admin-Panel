...
import { OgaStatCard, OgaAlert, OgaRecentActivity } from "../types";
...
export default function DashboardView() {
  ...
  const [statCards] = useState<OgaStatCard[]>(initialStatCards);
  const [alerts] = useState<OgaAlert[]>(initialAlerts);
  const [recentActivities] = useState<OgaRecentActivity[]>(
    initialRecentActivities,
  );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        {statCards.map((card) => (
          ...
          >
            <div className="">
              <div
                className={`w-8 h-8 rounded-lg flex items-center justify-center ${getIconBackground(card.iconType)}`}
              >
      ...
      <div className="space-y-3">
        ...
        {alerts.map((alert) => (
          ...
          >
            ...
            <button
              onClick={() => navigate(`/${alert.targetTab}`)}
      ...
      <div className="grid grid-cols-1  gap-5">
        ...
        <div className="lg:col-span-7 bg-white p-5 rounded-xl border border-emerald-950/5 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-2.5">
            <Link to="/agents">
              <button className="w-full max-w-75 flex items-center gap-3 px-4 py-2.5 bg-[#004d2c] hover:bg-[#00381e] text-white text-[10px] font-bold rounded-lg transition-colors cursor-pointer">
            ...
            <Link to="/reports">
              <button className="w-full max-w-75 flex items-center gap-3 px-4 py-2.5 bg-white hover:bg-slate-50 text-[#004d2c] border border-slate-200 text-[10px] font-bold rounded-lg transition-colors cursor-pointer">
